...
const SampleFilesPanel = ({ sample, files }: Props) => {
  const router = useRouter();
  const [isAdding, setIsAdding] = useState(false);

  const handleDelete = async (id: string) => {
    await researchActions.sampleFiles.delete({ id });
    router.refresh();
  };
  ...
  return (
    <Card.Root>
      ...
      {isAdding && (
        <Uploader
          pathPrefix={`research/sample-${sample.id}`}
          onComplete={async (data) => {
            await researchActions.sampleFiles.create({
              sampleId: sample.id,
              fileId: data.fileId,
            });
            setIsAdding(false);
